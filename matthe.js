...
client.on ( "voiceChannelLeave" , ( member , channel ) => {
    if ( member.user.bot ) return
    if (!member.roles.cache.has ("978716788374261830")) return     // Yetkili Rol ID ( KOMUTU KULLANABİLCEK )
    ...
    if ( data ) {
        ...
        else if ( channel.parentID == yayin ) {
            db.add ( `1yayin:${ member.user.id }` , new Date ().getTime () - Number ( data.start ) )
        }
    }
} );
...
client.on ( "voiceChannelSwitch" , ( member , oldChannel , newChannel ) => {
    ...
    if ( data ) {
        ...
        else if ( oldChannel.parentID == yayin ) {
            db.add ( `1streamer:${ member.user.id }` , new Date ().getTime () - Number ( data.start ) )
        }
...
client.on ( "message" , async message => {
    ...
    if ( data ) {
        ...
        if ( st.channel.parentID === pub ) {
            ...
            else if ( st.channel.parentID == yayin ) {
            db.add ( `1streamer:${ message.author.id }` , new Date ().getTime () - Number ( data.start ) )
        }  
...
//-----------------------------------------------------------------------------------------------------------------------------------\\
        ...
        let streamer1 = await db.fetch ( `1streamer:${ user.id }` ) || 0
        ...
        let toplamStreamer = moment.duration ( streamer1 ).format ( "h [saat] m [dakika.]" );
...
• Streamer Odaları: \`${ toplamStreamer }\`
